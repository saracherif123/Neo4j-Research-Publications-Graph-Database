...
def load_rel_published_in_conference(session):
    session.run(
        ...
        MATCH (p:Paper {PaperID: line.PaperID})
        MATCH (c:Conference {Name: line.ConferenceID})
...
def load_rel_published_in_journal(session):
    session.run(
        ...
        MATCH (p:Paper {PaperID: line.PaperID})
        MATCH (j:Journal {Name: line.JournalID})
...
def load_rel_published_in_workshop(session):
    session.run(
        ...
        MATCH (p:Paper {PaperID: line.PaperID})
        MATCH (w:Workshop {Name: line.WorkshopID})
...
def load_rel_reviews(session):
    session.run(
        ...
        MATCH (p:Paper {PaperID: line.PaperID})
        CREATE (a)-[:REVIEWS {Comment: line.Comment, Score: toInteger(line.Score)}]->(p)
        """
    )
...
if __name__ == '__main__':
    ...
    print("Done loading all data to Neo4j.")
    session.close()
